"""
Message and connection session handling class for use with TCP sessions. Inherit from this class 
and create handler methods using the @handler decorator. For example:
...
TODO:
-----
  - _register_message_handlers() is unable to check that the message name in handle_MessageName is
    actually spelled the same as in the @handler() decorator.
"""
...
def handler(message_type):
    def handler_decorator(func):
        ...
        async def handler_wrapper(self, session, dictionary: Dict[str, Any], timestamp: float):
            try:
                # Decode message and pass to actual wrapped handler
                msg = deserialize(message_type=message_type, dictionary=dictionary)
...
class MessageHandler:
    ...
    async def handle_message(self, session, json_string: str, timestamp: float):
